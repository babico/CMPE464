...
public:
	...
	NoisyImage() = default;
	NoisyImage(const std::string _image_path, const std::string _image_name, cv::Rect _selected_part)
		: image_name(_image_name.substr(0, _image_name.find("."))),
		  selected_part(_selected_part)
	{
		// Get image extension
...
const std::string IMAGES_PATH = "/home/babico/Projects/CMPE464-Github/HW4/images/";
using namespace matplot;

int main()
{
	// Create noisy images with selected parts
	std::vector<NoisyImage> noisy_images = {
		NoisyImage(IMAGES_PATH, "noisy1.jpg", cv::Rect(315, 0, 195, 140)),
		NoisyImage(IMAGES_PATH, "noisy2.jpg", cv::Rect(315, 0, 195, 140)),
		NoisyImage(IMAGES_PATH, "noisy3.jpg", cv::Rect(0, 95, 95, 170)),
	};
	...
	// noisy1 image noise type is pattern noise
	// Fix with bilateral x gaussian filter
	cv::bilateralFilter(noisy_images[0].image_gray, noisy_images[0].denoised_image, 8, 256, 256);
	cv::GaussianBlur(noisy_images[0].denoised_image, noisy_images[0].denoised_image, cv::Size(5, 5), 2);
	cv::imwrite(IMAGES_PATH + "denoised_bilateral_" + noisy_images[0].image_name + ".png", noisy_images[0].denoised_image);

	// noisy2 image noise type is pattern noise
	cv::bilateralFilter(noisy_images[1].image_gray, noisy_images[1].denoised_image, 8, 256, 256);
	cv::GaussianBlur(noisy_images[1].denoised_image, noisy_images[1].denoised_image, cv::Size(3, 3), 2);
	cv::imwrite(IMAGES_PATH + "denoised_bilateral_" + noisy_images[1].image_name + ".png", noisy_images[1].denoised_image);

	// noisy3 image noise type is pattern noise
	cv::bilateralFilter(noisy_images[2].image_gray, noisy_images[2].denoised_image, 8, 256, 256);
	cv::GaussianBlur(noisy_images[2].denoised_image, noisy_images[2].denoised_image, cv::Size(3, 3), 4);
	cv::imwrite(IMAGES_PATH + "denoised_bilateral_" + noisy_images[2].image_name + ".png", noisy_images[2].denoised_image);
